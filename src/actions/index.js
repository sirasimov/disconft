	...
	return async (dispatch) => {
		try {
			const response = await axios.get(
				`https://api.hangzhounet.tzkt.io/v1/contracts/${config.contractAddress}/bigmaps/data/keys`
			);
			const response1 = await axios.get(
				`https://api.hangzhounet.tzkt.io/v1/contracts/${config.tokenAddress}/bigmaps/token_metadata/keys`
			);
